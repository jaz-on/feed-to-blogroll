...
class Feed_To_Blogroll_Template {
	...
	public function blogroll_shortcode( $atts ) {
		...
		$cache_key = 'blogroll_' . $atts['category'] . '_' . $atts['limit'] . '_' . $atts['columns'] . '_v' . get_option( 'feed_to_blogroll_cache_version', 1 );
	...
	private function prepare_blog_data( $blogs ) {
		...
		foreach ( $blogs as $blog ) {
			$blog_data[] = array(
				...
				'description' => $blog->post_excerpt ?: wp_trim_words( $blog->post_content, 20 ),
		...
		<div class="<?php echo esc_attr( $container_class ); ?>" role="region" aria-label="<?php esc_attr_e( 'Blogroll', 'feed-to-blogroll' ); ?>">
			<?php if ( $atts['show_export'] ) : ?>
				<div class="blogroll-export" role="toolbar" aria-label="<?php esc_attr_e( 'Blogroll export options', 'feed-to-blogroll' ); ?>">
					<button type="button" class="export-opml-button" data-nonce="<?php echo esc_attr( wp_create_nonce( 'feed_to_blogroll_export' ) ); ?>">
							...
							<h3 class="blog-title">
								<?php if ( ! empty( $blog['site_url'] ) ) : ?>
									<a href="<?php echo esc_url( $blog['site_url'] ); ?>" target="_blank" rel="noopener noreferrer">
										...
										<span class="external-link-icon" aria-label="<?php esc_attr_e( 'Opens in new window', 'feed-to-blogroll' ); ?>">
	...
	public function get_blogroll_rest( $request ) {
		...
		return new WP_REST_Response( $blog_data, 200 );
	}
	...
	public function export_opml_frontend() {
		// Check nonce
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'feed_to_blogroll_export' ) ) {
